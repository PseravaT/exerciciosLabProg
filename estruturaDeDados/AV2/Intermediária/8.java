...
class Grafo{
    ...
    void removerVertice(String vertice) {
        if (grafo.containsKey(vertice)) {
            ...
            System.out.println("\nVértice '" + vertice + "' removido com sucesso, patrão!");
        } else {
            System.out.println("\nEsse vértice não existe, parceiro.");
        }
    }

    void removerAresta(String a, String b){
        if (!grafo.containsKey(a) || !grafo.containsKey(b)){
            System.out.println("um deles não existe");
            return;
        }
        boolean tinhaLigacao = false;

        if (grafo.get(a).remove(b)) {
            tinhaLigacao = true;
        }

        if (grafo.get(b).remove(a)) {
            tinhaLigacao = true;
        }

        if (tinhaLigacao) {
            System.out.println("\nAresta entre '" + a + "' e '" + b + "' foi removida.");
        } else {
            System.out.println("\nNão havia aresta entre '" + a + "' e '" + b + "'.");
        }
    }
    ...
    void contador (){
        ...
        System.out.println("\nTotal de vértices: " + totalVertices);
    ...
    void semVizinho(){
        ...
        for (String v:grafo.keySet()){
            if (grafo.get(v).isEmpty()){
                System.out.println("\nVértice '" + v +"'Está sem vizinho" );
                status = true;
            }
        }
        if (!status) {
            System.out.println("\nNenhum vértice isolado encontrado.");
        }
    }
}
...
//---------------------------------------------------------------------------------------//
        ...
        System.out.print("\nDigite o Primeiro vértice para remover a aresta: ");
        String origem = scanner.next();
        System.out.print("\nDigite o segundo vértice para remover a aresta: ");
        String destino = scanner.next();

        g.removerAresta(origem, destino);
        g.mostrarGrafo();
//---------------------------------------------------------------------------------------//
